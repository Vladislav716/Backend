...
from app.core.enums import Visibility,UserRoleTypes
...
def search_filter_sort_paginate(
    ...
    user_role: UserRoleTypes = UserRoleTypes.user,
):
    """Common functionality for searching, filtering and sorting"""
    model_cls = get_class_by_tablename(model)
    sort_spec = create_sort_spec(get_model_name_by_tablename(model), sort_by, descending)
    ...
    if not filter_spec:
        filter_spec = create_filter_spec(get_model_name_by_tablename(model), fields, ops, values, user_role)
